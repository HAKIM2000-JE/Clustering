...
def get_Recommendation(propertyBookingId):
    ...
    #Profiling Service
    ProflingResult = Profiling.getProfiles(CloseReservation)
...
def getProfileRecommendation(ListProfileId, CloseRecommendation):
    ...
    for index, row in CloseRecommendation.iterrows():
        ...
        if not ServiceData.get_guestReviews({"_id.recommendationId": row['_id']}).empty:
                        ...
                        print("tt", Recommandation_guestReviews['_id'].tolist()[0]['guestId'])
    ...
    for element in List:
        ...
        print( "Title : ", ServiceData.get_Recommendation_DataFrame({'_id':element['Recommendation Id']})['title'])
        print("Adresse : ", ServiceData.get_Recommendation_DataFrame({'_id': element['Recommendation Id']})['fullAddress'])
        RecommendationData['Recommendation Id'] = element['Recommendation Id']
        RecommendationData['Title']=ServiceData.get_Recommendation_DataFrame({'_id':element['Recommendation Id']})['title']
        RecommendationData['Adresse'] = ServiceData.get_Recommendation_DataFrame({'_id': element['Recommendation Id']})[
            'fullAddress']
        #RecommendationData['description'] = ServiceData.get_Recommendation_DataFrame({'_id': element['Recommendation Id']})['description']
